...
namespace RoomManagement
{
    ...
    public class RoomOperations
    {
        ...
        public void BinarySearchStudentById(string studentId)
        {
            ...
            string studentInfoFilePath = "StudentInfo.txt";
            string roomNumber = null;
            if (!File.Exists(studentInfoFilePath))
            {
                Console.WriteLine("File StudentInfo.txt không tồn tại.");
                return;
            }
            ...
            try
            {
                ...
                // Đọc từng dòng trong file và kiểm tra mã số sinh viên
                var lines = File.ReadAllLines(studentInfoFilePath);
                ...
                foreach (var line in lines)
                {
                    ...
                    // Kiểm tra xem dòng có đủ 3 phần tử và mã sinh viên có trùng khớp không
                    if (parts.Length >= 3 && parts[3].Trim() == studentId)
                    {
            ...
            while (left <= right)
            {
                ...
                if (comparison == 0) // Tìm thấy sinh viên
                {
                    ...
                    Console.WriteLine("-------------------------------------------------------------------");
                    Console.WriteLine($"| {student[1],-20} | {student[2],-25}|{roomNumber}            |");
                    Console.WriteLine("-------------------------------------------------------------------");
                    ...
                    Console.WriteLine($"Lớp: {student[6]}   ");
                    found = true;
                    break;
                }
        ...
        public static void RegisterStudentRoom()
        {
            string signRoomFilePath = "SignRoom.txt";
            string studentInfoFilePath = "StudentInfo.txt";

            if (!File.Exists(signRoomFilePath))
            {
                Console.WriteLine("File SignRoom.txt không tồn tại.");
                return;
            }
            ...
            {
                Console.Clear();
                Console.WriteLine("=== DANH SÁCH YÊU CẦU ĐĂNG KÝ PHÒNG ===");
                Console.WriteLine("==============================================");
                Console.WriteLine("| {0,-10} | {1,-15} | {2,-15} |", "Số Phòng", "Tên", "Mã SV");
                Console.WriteLine("==============================================");

                // Đọc và hiển thị các yêu cầu đăng ký phòng trong file SignRoom.txt
                var lines = File.ReadAllLines(signRoomFilePath);
                foreach (var line in lines)
                {
                    var parts = line.Split(',');

                    // Chỉ in thông tin nếu dòng đó có ít nhất 4 phần tử và bắt đầu bằng ký tự "2"
                    if (parts.Length >= 4)
                    {
                        string roomNumber1 = parts[1];
                        string name1 = parts[2];
                        string studentId1 = parts[3];

                        Console.WriteLine("| {0,-10} | {1,-15} | {2,-15} |", roomNumber1, name1, studentId1);
                    }
                }
                ...
                // Định dạng thông tin để lưu vào file StudentInfo.txt
                string newStudentInfo = $"2,{roomNumber},{name},{studentId}";

                // Lưu thông tin vào file StudentInfo.txt
                File.AppendAllText(studentInfoFilePath, newStudentInfo + Environment.NewLine);
                Console.WriteLine("Thông tin sinh viên đã được lưu ");

                // Xóa dòng có mã sinh viên tương ứng trong file SignRoom.txt
                var updatedLines = lines.Where(line =>
                {
                    var parts = line.Split(',');
                    // Xóa dòng nếu mã sinh viên trong dòng trùng với mã sinh viên đã nhập
                    return !(parts.Length >= 4 && parts[0] == "2" && parts[3] == studentId);
                }).ToList();

                // Kiểm tra và cập nhật file nếu dòng cần xóa đã tồn tại
                if (updatedLines.Count < lines.Length)
                {
                    File.WriteAllLines(signRoomFilePath, updatedLines);
                    
                }
                else
                {
                    Console.WriteLine("Không tìm thấy yêu cầu đăng ký phòng của sinh viên trong SignRoom.txt.");
                }
