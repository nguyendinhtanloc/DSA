...
namespace RoomRegistrationStudent
{
    public class RoomRegistration
    {
        ...
        public static void FindRoom2(string roomCode)
        {
            ...
            foreach (string line in lines)
            {
                ...
                if (parts.Length == 4 && parts[0].Trim().Equals(roomCode, StringComparison.OrdinalIgnoreCase))
                {
                    ...
                    Console.WriteLine("===============================================================================");
                    Console.WriteLine("| {0,-10} | {1,-10} | {2,-10}|", "Mã phòng", "Tên phòng", "Số tiền (Y: Đã thanh Toán / N: Chưa thanh toán) ");
                    Console.WriteLine("===============================================================================");
        ...
        public static void RegisterRoom(string username)
        {
            try
            {
                Console.Clear();
                Console.WriteLine("=== YÊU CẦU ĐĂNG KÝ PHÒNG ===");

                // Lấy mã số sinh viên từ username hiện tại
                string studentId = username.Substring(0, 8);

                // Nhập thông tin đăng ký phòng
                ...
                string registrationInfo = $"2,{roomNumber},{name},{studentId}";

                // Lưu thông tin vào file SignRoom
                string filePath = "SignRoom.txt";
                File.AppendAllText(filePath, registrationInfo + Environment.NewLine);

                Console.WriteLine("Yêu cầu đăng ký phòng đã được gửi!");
    ...
    public static class RoomRegistrationMenu
    {
        public static void ShowRoomRegistrationMenu(RoomOperations roomOperations)
        {
            ...
            while (true)
            {
                ...
                Console.WriteLine("| 2. Yêu cầu đăng ký phòng          |");
                ...
                switch (choice)
                {
                    ...
                    case 2:
                        RoomRegistration.RegisterRoom(first8Chars);
                        break;
